...
def load_data(dataset_path, resolution, dataset, pid_num, pid_shuffle, cache=True):
    ...
    for _label in sorted(list(os.listdir(dataset_path))): # _label: people id
        # # In CASIA-B, data of subject #5 is incomplete.
        # # Thus, we ignore it in training.
        # if dataset == 'CASIA-B' and _label == '005':
        #     continue
        label_path = osp.join(dataset_path, _label).replace("\\", "/")
        for _seq_type in sorted(list(os.listdir(label_path))): # _seq_type: walking condtition
            seq_type_path = osp.join(label_path, _seq_type).replace("\\", "/")
            for _view in sorted(list(os.listdir(seq_type_path))): # _view: in our data 0 3 6 9
                _seq_dir = osp.join(seq_type_path, _view).replace("\\", "/")
